...
contract PerpV2Controller {
  ...
  {
    return
      uint256(sqrtPriceX96).mul(uint256(sqrtPriceX96).mul(MAX_BPS)) >> (96 * 2);
  }
  ...
  function getTotalPerpPositionSize() public view returns (int256) {
    return
      accountBalance.getTotalPositionSize(address(this), address(baseToken));
  }
  ...
  function _openPositionByAmount(
    ...
    return
      accountBalance.getTakerPositionSize(address(this), address(baseToken));
  }
