...
contract Batcher is IBatcher, EIP712, ReentrancyGuard {
   ...
   * @param recipient Address to allot the LP tokens to
   */
  function depositFunds(uint256 amountIn, bytes memory signature, address recipient)
    external
    override
    nonReentrant
  {
   ...
   * @param amountOut Amount of token to be withdrawnnpz
   * @param recipient Address to receive the want tokens
   */
  function completeWithdrawal(uint256 amountOut, address recipient) external override nonReentrant {
  ...
  function validDeposit(address recipient, bytes memory signature) internal view {

    if (checkValidDepositSignature) {
      require(
        verifySignatureAgainstAuthority(recipient, signature, verificationAuthority),
        "INVALID_SIGNATURE"
      );
    }
